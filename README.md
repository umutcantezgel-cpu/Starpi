...
flowchart TD
    subgraph Client Initialization
        ...
        B ==>|Query GPU Adapter Limits| C{Inspect navigator.gpu and limits}
        C ==>|maxBufferSize >= 1GB and RAM >= 8GB| D[Tier: Desktop High Precision]
        C ==>|Mobile Device or Constrained Memory| E[Tier: Mobile Ultra Compact]
    ...
    subgraph Metadata & Registry
        ...
        E ==> G
        F ==> G
        G ==>|Resolve MLC Model Artifacts and Overrides| H[Runtime Config Generator]
        H ==>|Define Context Window and Prefill Chunks| I[Engine Config Payload]
    end

    subgraph Weight Streaming & Storage
        I ==> J[Weight Streaming Controller]
        J ==> K{Cache API or IndexedDB Check}
        K ==>|Hit: Pre cached Shards| L[Load Tensor Shards from Local Cache]
        K ==>|Miss: Network Request| M[Stream Binary Tensor Shards from CDN]
        M ==>|Write to Local Cache API| L
    end

    subgraph WebGPU Execution & RAG
        L ==> N[GPU Buffer Allocation and Shader Compilation]
...
2. **Model Metadata Registry**: Maps dynamic tiers to model definitions, tokenizer configurations, and execution overrides. Ensures context window and prefill sizes prevent out of memory browser tab termination.
3. **Weight Streaming Controller**: Coordinates asynchronous, chunked binary weight downloads from Hugging Face or MLC repositories. Automatically persists downloaded weights within browser storage (Cache API and IndexedDB).
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Llama 3.2 1B Instruct (q4f16_1) | 1.23B | 4 bit weight, 16 bit float activation | 705 MB | ~879 MB | Mobile or Constrained |
| Qwen 2.5 1.5B Instruct (q4f16_1) | 1.54B | 4 bit weight, 16 bit float activation | 980 MB | ~1.5 GB | Low tier Desktop or Tablet |
| DeepSeek R1 Distill Qwen 1.5B (q4f16_1) | 1.54B | 4 bit weight, 16 bit float activation | 954 MB | ~1.5 GB | Mobile High Reasoning |
| Qwen 2.5 3B Instruct (q4f16_1) | 3.09B | 4 bit weight, 16 bit float activation | 1.85 GB | ~2.5 GB | Desktop Auto or High Precision |
| Hermes 3 Llama 3.2 3B (q4f16_1) | 3.21B | 4 bit weight, 16 bit float activation | 1.80 GB | ~2.4 GB | Desktop Autonomous Logic |
| Phi 3.5 mini Instruct (q4f16_1) | 3.82B | 4 bit weight, 16 bit float activation | 2.20 GB | ~2.9 GB | High Density Reasoning |

### Runtime & System Requirements

* **Browser WebGPU Support**:
  * Google Chrome or Chromium version 113 or higher (Desktop: Windows, macOS, Linux)
  * Google Chrome version 121 or higher (Android with WebGPU support enabled)
  * Apple Safari version 17.4 or higher (iOS, iPadOS, macOS with WebGPU active)
* **GPU Capabilities**:
  * WebGPU adapter supporting compute shaders and float16 shader extensions (shader f16 where available).
  * Desktop tier requirement: maxBufferSize 1073741824 bytes (1 GB) and deviceMemory 8 GB.
  * Mobile tier requirement: maxBufferSize 268435456 bytes (256 MB) with 128 token prefill chunking.
* **Storage and Caching**:
...
* Modern browser with native WebGPU support (Chrome, Edge, or Safari version 17.4 or higher)
...
1. Provision a PostgreSQL instance (such as Supabase, Neon, or self hosted PostgreSQL).
...
* **Continuous Automated Benchmarking**:
  ...
  * Automated measurement of tokens per second, time to first token, and memory footprint across varied device tiers.
...
* **Shader Kernel Optimization**:
  * Investigation of cooperative matrix multiplications (subgroups or wgpu matrix) to maximize memory bandwidth utilization on integrated Apple Silicon and Vulkan backends.
